...
//	private RectF mChangePageRectLeft;
//	private RectF mChangePageRectRight;
	// View mode.
	...
	private float  coverMarginX = 0.f;
	private float  coverMarginTop = 0.f;
	private float  coverMarginBottom = 0.f;
	...
	/***
	 * 속지 크기를 border 만큼 작게 하는 함수...
	 * @param isFlag
	 */
	public void setCoverMode(boolean isFlag){
		mIsCover = isFlag;
	}
	/**
	...
	public RectF getPageRect(int page, int currentPage, int allPage) {
		boolean isContent = false;
		if (page == PAGE_LEFT) {
			RectF temp = new RectF();
			temp.set(mPageRectLeft);
			
			if(currentPage > 1 && currentPage < allPage)
				isContent = true;
			
			
			if(mIsCover && isContent){
				
				
				temp.left += (coverMarginX * mPageRectLeft.width());
				temp.top += (coverMarginTop * mPageRectLeft.height());
				temp.bottom -= (coverMarginBottom * mPageRectLeft.height());
			}
			...
			RectF temp = new RectF();
			temp.set(mPageRectRight);
			if(currentPage > 0 && currentPage < allPage-1)
				isContent = true;
			
			if(mIsCover && isContent){
				
				
				temp.right -= (coverMarginX * mPageRectRight.width());
				temp.top += (coverMarginTop * mPageRectRight.height());
				temp.bottom -= (coverMarginBottom * mPageRectRight.height());
			}
