...
public class CurlMesh {
	...
	public synchronized void curl(PointF curlPos, PointF curlDir, double radius, boolean isFront) {
		mIsFront = isFront;
		curl(curlPos, curlDir, radius);
	}
	...
	public synchronized void curl(PointF curlPos, PointF curlDir, double radius) {
		...
		int lines[][];
		if (CurlView.IS_CURL) {
			int liness[][] = {{0, 1}, {0, 2}, {1, 3}, {2, 3}};
			lines = liness;
		} else {
			int liness[][] = {{0, 0}, {0, 0}, {0, 0}, {0, 0}};
			lines = liness;
		}
		{
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		if (DRAW_SHADOW) {
			...
			gl.glColorPointer(4, GL10.GL_FLOAT, 0, mBufShadowColors);
			gl.glVertexPointer(3, GL10.GL_FLOAT, 0, mBufShadowVertices);
			gl.glDrawArrays(GL10.GL_TRIANGLE_STRIP, mDropShadowCount, mSelfShadowCount);
			gl.glDisableClientState(GL10.GL_COLOR_ARRAY);
			gl.glDisable(GL10.GL_BLEND);
		}
