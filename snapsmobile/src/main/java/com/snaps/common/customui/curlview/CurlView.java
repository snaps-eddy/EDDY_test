...
package com.snaps.common.customui.curlview;
...
public class CurlView extends GLSurfaceView implements View.OnTouchListener, CurlRenderer.Observer {
	...
	public interface OnCurledListener {
		public void onCurledToPage(int position, boolean isMove);

		/***
		 * curl이 시작이 되면 호출이된다. 현재 페이지, curl방향..
		 * 
		 * @param position
		 * @param directon
		 */
		public void onStartCurl(int currentIdx, int directon);
	}

	private OnCurledListener mOnCurledListener;

	public void setOnCurledListener(OnCurledListener onCurledListener) {
		mOnCurledListener = onCurledListener;
	}

	public static boolean IS_CURL = false;
	public int curlmode = 0;
	...
	private OnFirstEndPageTouch mOnFirstEndPageTouch;

	// One page is the default.
	private int mViewMode = SHOW_ONE_PAGE;

	/** flip과 curl 섞는경우 curl의 처음과 끝일 경우 curl하지 않고 event를 flip으로 넘긴다. */
	private boolean isFirstLastJump = false;

	// flip할때 앞면을 그릴지 뒤면을 그릴지 결
	private boolean isFront = true;
	
	public static boolean iscurling = false;
	...
	public CurlView(Context ctx, AttributeSet attrs) {
		...
		boolean isTransparent = a.getBoolean(R.styleable.CurlView_transparent, false);

		init(ctx, isTransparent);
	...
	public void setFirstLastJump(boolean isFirstLastJump) {
		this.isFirstLastJump = isFirstLastJump;
	}

	public void setNoCurl(boolean bnoCurl) {
		noCurl = bnoCurl;
	}
	...
	public boolean getAnimate() {
		boolean getanimate; 
		getanimate = this.mAnimate;
		
		return getanimate;
	}

	/***
	 * 하드커버인 경우 속지와 커버 사이즈를 차이가 나게 함..
	 * 
	 * @param isEnable
	 */
	public void setCoverMode(boolean isEnable) {
		mRenderer.setCoverMode(isEnable);
	}
	 ...
	 * @param b
	 */
	public void setCoverMargin(float x, float t, float b) {
		mRenderer.setCoverMargin(x, t, b);
	}

	/**
	 * Initialize method.
	 */
	private void init(Context ctx, boolean isTransparent) {
	...
	public void setCurlCoverRatio(float x, float t, float b) {
		mRenderer.setCoverMargin(x, t, b);
	}

	public void setCurlCoverMode(boolean isFlag) {
		mRenderer.setCoverMode(isFlag);
	}
	...
	public void onDrawFrame() {
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			...
			if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
				...
				if (mCurrentIndex == 1 && mCurlState == CURL_LEFT) {
					right.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, 0, mPageProvider.getPageCount()));
				} else if (mCurrentIndex == 1 && mCurlState == CURL_RIGHT) {
					right.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, 1, mPageProvider.getPageCount()));
				} else if (mCurlState == CURL_LEFT && mCurrentIndex == mPageProvider.getPageCount() - 1) {
					right.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, 1, mPageProvider.getPageCount()));
				} else
					right.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, mCurrentIndex, mPageProvider.getPageCount()));
				...
				if ((mCurlState == CURL_RIGHT && mCurrentIndex == 1) || (mCurlState == CURL_RIGHT && mCurrentIndex == mPageProvider.getPageCount() - 1)) {
					left.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex + 1, mPageProvider.getPageCount()));
				} else if (mCurlState == CURL_LEFT && mCurrentIndex == mPageProvider.getPageCount() - 1) {
					// 영역이 작아야 한다.
					left.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, 2, mPageProvider.getPageCount()));
				} else
					left.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex, mPageProvider.getPageCount()));
			...
			Dlog.d("onDrawFrame() currentIndex:" + mCurrentIndex);
			if (mOnCurledListener != null)
				mOnCurledListener.onCurledToPage(mCurrentIndex, isMove);
			if (!isMove)
				noCurl = false;
			...
			// 앞면이 보여야 할지 뒤면이 보여야할지 판다.
			if (!IS_CURL) {
				RectF rightRect = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, mCurrentIndex, mPageProvider.getPageCount());
				if (mCurlState == CURL_LEFT) {
	...
	public void setCurlMode(int curl) {
		IS_CURL = true;
		...
		if (curlmode == 0) {
			...
			if ((mCurrentIndex == 0 && mCurlState == CURL_RIGHT) || (mCurrentIndex == 1 && mCurlState == CURL_LEFT)) {
				IS_CURL = true;
			} else if ((mCurrentIndex == mPageProvider.getPageCount() - 1 && mCurlState == CURL_RIGHT) || (mCurrentIndex == mPageProvider.getPageCount() && mCurlState == CURL_LEFT)) {
				IS_CURL = true;
			}
	...
	public boolean onTouch(View view, MotionEvent me) {
		...
		RectF rightRect = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, mCurrentIndex, mPageProvider.getPageCount());
		RectF leftRect = mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex, mPageProvider.getPageCount());
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (mDragStartPos.y > rightRect.top || mDragStartPos.y < rightRect.bottom) {
				return false;
			} else {
				if (noCurl) {// 페이지가 넘어가는 중이거나, 이미지 로딩중일경우엔 다음장으로 넘기는 액션을 받지 않음.
                    return false;
				}

				iscurling = true;
				noCurl = true;// 페이지를 잡자마자 다음 액션 막음

				mFixedY = mPointerPos.mPos.y;
				Dlog.d("onTouch() currentIndex:" + mCurrentIndex);
				...
				if (mViewMode == SHOW_TWO_PAGES) {
					...
					if (mDragStartPos.x < rightRect.left && mCurrentIndex > 0) {
						Dlog.d("onTouch() CURL_LEFT");

						mDragStartPos.x = leftRect.left;

						startCurl(CURL_LEFT);
						// curl 여부 체크
						setCurlMode(curlmode);
					}
		...
		case MotionEvent.ACTION_UP: {
			if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
				// if (noCurl) {
				// mPointerPos.mPos.x = mCurlState == CURL_RIGHT ? -1000 :
				// 1000;
				// mPointerPos.mPos.y = mFixedY;
				// }
				...
				if (mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x > (rightRect.left + rightRect.right) / 2 || mViewMode == SHOW_TWO_PAGES
						&& mPointerPos.mPos.x > (mCurlState == CURL_RIGHT ? (rightRect.left + rightRect.right) / 2 : (leftRect.left + leftRect.right) / 2)) {
					...
					if (mCurlState == CURL_RIGHT || mViewMode == SHOW_TWO_PAGES) {
						if (mCurrentIndex == 1)
							...
							mAnimationTarget.x = -rightRect.right;// leftRect.left;
																	// 수정
																	// 애니메이션
																	// 때문에...
					} else {
	...
	private void setCurlPos(PointF curlPos, PointF curlDir, double radius) {
		...
		if (mCurlState == CURL_RIGHT || (mCurlState == CURL_LEFT && mViewMode == SHOW_ONE_PAGE)) {
			RectF pageRect = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, mCurrentIndex, mPageProvider.getPageCount());
			...
			RectF pageRect = mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex, mPageProvider.getPageCount());
			if (curlPos.x <= pageRect.left) {
				mPageCurl.reset();
				requestRender();
				return;
		...
		double dist = IS_CURL ? Math.sqrt(curlDir.x * curlDir.x + curlDir.y * curlDir.y) : (mCurlState == CURL_RIGHT) ? -curlDir.x : curlDir.x;
		if (dist != 0) {
			curlDir.x /= dist;
			curlDir.y /= dist;
			if (CurlView.IS_CURL) {
				...
				mPageCurl.curl(curlPos, curlDir, radius, isFront);
			}
		} else {
			mPageCurl.reset();
		}
	...
	public void setCurrentIndex(int index) {
		Dlog.d("setCurrentIndex() index:" + index);
	...
	private void startCurl(int page) {
		...
		if (mOnCurledListener != null)
			mOnCurledListener.onStartCurl(mCurrentIndex, page);
		...
		case CURL_RIGHT: {
			...
			if (mCurrentIndex > 0) {
				mPageLeft.setFlipTexture(true);
				mPageLeft.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex, mPageProvider.getPageCount()));
				mPageLeft.reset();
				if (mRenderLeftPage) {
			...
			if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
				...
				mPageRight.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, mCurrentIndex + 1, mPageProvider.getPageCount()));
				mPageRight.setFlipTexture(false);
				mPageRight.reset();
			...
			mPageCurl.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, mCurrentIndex, mPageProvider.getPageCount()));
		...
		case CURL_LEFT: {
			...
			if (mCurrentIndex > 1) {
				...
				mPageLeft.setFlipTexture(true);
				if (mCurrentIndex == 2 || mCurrentIndex == mPageProvider.getPageCount()) {
					mPageLeft.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex - 1, mPageProvider.getPageCount()));
				} else
					mPageLeft.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex, mPageProvider.getPageCount()));
				mPageLeft.reset();
				if (mRenderLeftPage) {
			...
			if (mCurrentIndex < mPageProvider.getPageCount()) {
				mPageRight.setFlipTexture(false);
				mPageRight.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, mCurrentIndex, mPageProvider.getPageCount()));
				mPageRight.reset();
				mRenderer.addCurlMesh(mPageRight);
			}
			...
			if (mViewMode == SHOW_ONE_PAGE || (mCurlState == CURL_LEFT && mViewMode == SHOW_TWO_PAGES)) {
				...
				mPageCurl.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT, mCurrentIndex, mPageProvider.getPageCount()));
	...
	private void updateCurlPos(PointerPosition pointerPos) {
		...
		if (mAnimate && mCurrentIndex == mPageProvider.getPageCount() - 1 && mCurlState == CURL_LEFT) {
			currentIndex = 1;
		}
		// Default curl radius.
		double radius = mRenderer.getPageRect(CURL_RIGHT, currentIndex, mPageProvider.getPageCount()).width() / 3;
		// if (noCurl)
		// radius = 0;
		...
		if (mCurlState == CURL_RIGHT || (mCurlState == CURL_LEFT && mViewMode == SHOW_TWO_PAGES)) {
			...
			float pageWidth = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, currentIndex, mPageProvider.getPageCount()).width();
			...
			if (dist >= curlLen) {
				...
				if (mViewMode == SHOW_TWO_PAGES) {
					...
					float pageLeftX = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, currentIndex, mPageProvider.getPageCount()).left;
		...
		// Otherwise we'll let curl follow pointer position.
			...
			float pageRightX = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT, currentIndex, mPageProvider.getPageCount()).right;
	...
	public interface OnFirstEndPageTouch {
		public void onFirstPageTouch();

		public void onEndPageTouch();
	}

	public void setOnEndPageTouch(OnFirstEndPageTouch onTouch) {
		this.mOnFirstEndPageTouch = onTouch;
	}
