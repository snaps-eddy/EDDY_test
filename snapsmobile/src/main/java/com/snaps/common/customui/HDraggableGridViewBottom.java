...
public class HDraggableGridViewBottom extends HDraggableGridParent {
	private static final String TAG = HDraggableGridViewBottom.class.getSimpleName();
	// layout vars
	public static float childRatio = .9f;
	protected int colCount = 4, childSize, padding, dpi, scroll = 0;
	protected int minChildHeight = 0;
	protected int minChildWidth = 0;
	...
	public HDraggableGridViewBottom(Context context, AttributeSet attrs) {
		super(context, attrs);
		setListeners();
		...
		boolean isVerticalCalendar = Config.isCalendarMini(Config.getPROD_CODE()) || Config.isCalendarVert(Config.getPROD_CODE()) || Config.isCalendarNormalVert(Config.getPROD_CODE());
		if (isVerticalCalendar) {
			minChildHeight = 0;
			minChildWidth = 0;
		} else {
			minChildHeight = UIUtil.convertDPtoPX(context, 80);
			minChildWidth = UIUtil.convertDPtoPX(context, 60);
		}
	}
	...
	protected Runnable updateTask = new Runnable() {
		public void run() {
			if (dragged != -1)// 드래그모드
			{
				if (lastX < padding * 4 && scroll > 0) {
					scroll -= 20;
				} else if (lastX > getRight() - getLeft() - (padding * 4) && scroll < getMaxScroll()) {
					scroll += 20;
				}
		...
		if (isAnimation) {
			...
			fadeOut.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					HDraggableGridViewBottom.super.removeViewAt(index);
					newPositions.remove(index);
				}
			});
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		boolean isVerticalCalendar = Config.isCalendarMini(Config.getPROD_CODE()) || Config.isCalendarVert(Config.getPROD_CODE()) || Config.isCalendarNormalVert(Config.getPROD_CODE());
		if (setchildSize != 0 && !isVerticalCalendar) {
			childSize = Math.max(setchildSize, minChildWidth);
		} else {
			if (childSize <= 0) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			if (i != dragged) {
				...
				Point xy = getCoorFromIndex(childSize, i);
				...
				getChildAt(i).layout(xy.x, xy.y, xy.x + childSize, xy.y + minChildHeight);
				getChildAt(i).measure(MeasureSpec.makeMeasureSpec(childSize, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(minChildHeight, MeasureSpec.EXACTLY));
			}
		}
	}
	...
	public int getIndexFromCoor(int x, int y) {
		int col = getColOrRowFromCoor(x + scroll), row = 0;//getColOrRowFromCoor(y);
	...
	protected Point getCoorFromIndex(int offestSize, int index) {
		return new Point(padding + (offestSize + padding) * index - scroll, padding);// 무조건
		// 1줄
	}
	...
	public void scrollToIdx(int moveType, int childIdx) {
		...
		switch (moveType) {
			case EditActivityThumbnailUtils.PAGE_MOVE_TYPE_NEXT:
				targetIdx += 1;
				smoothScrollTarget = targetIdx * childSize + (targetIdx + 1) * padding - getWidth();
				break;
			case EditActivityThumbnailUtils.PAGE_MOVE_TYPE_PREV:
				targetIdx += 4;
				smoothScrollTarget = targetIdx * childSize + (targetIdx + 1) * padding - getWidth();
