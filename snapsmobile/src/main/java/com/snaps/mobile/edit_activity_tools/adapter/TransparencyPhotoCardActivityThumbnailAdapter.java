 ...
 */
public class TransparencyPhotoCardActivityThumbnailAdapter extends CardShapeActivityThumbnailAdapter {
    private static final String TAG = TransparencyPhotoCardActivityThumbnailAdapter.class.getSimpleName();

    public TransparencyPhotoCardActivityThumbnailAdapter(Activity activity, SnapsProductEditInfo productEditInfo) {
        super(activity, productEditInfo);
    }

    @Override
    public CardShapeActivityThumbnailHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        int layoutId = isLandscapeMode ? R.layout.transparcency_photo_card_bottomview_item_renewal_horizontal_dragable : R.layout.transparcency_photo_card_bottomview_item_renewal_dragable;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        ...
        SnapsPage paage = m_arrPageList.get(THUMBNAIL_CELL_POSITION);
        ...
        leftThumbnailView.setPage(paage);
        setThumbnailHolder(leftThumbnailView, THUMBNAIL_CELL_POSITION);



        holder.getCounterView().setText(String.valueOf(paage.getQuantity()));
        ...
        holder.getCounterView().setTextColor((THUMBNAIL_CELL_POSITION == currentPageIndex) ? Color.argb(255, 229, 71, 54) : Color.argb(186, 186, 186, 186));
    }
    ...
    protected void bottomViewItemClick(int position, CardShapeActivityThumbnailHolder holder, boolean isClick) {
        ...
        CardShapeThumbnailChildView leftThumbnailView = getThumbnailView(holder, CardShapeThumbnailChildView.eCARD_SHAPE_THUMBNAIL_PAGE_TYPE.CARD_SHAPE_THUMBNAIL_TYPE_PAGE_LEFT);
        int drawable = isClick ? R.drawable.shape_image_border_transparency_photo_card_select : R.drawable.shape_image_tranparency_photo_card_border;
    ...
    protected void setThumbnailDimensions(CardShapeActivityThumbnailHolder holder, Rect thumbnailRect) {
        if (thumbnailRect != null) {
            ...
            int rectWidth = thumbnailRect.width() / 2 ;
    ...
    protected void setThumbnailHolder(final CardShapeThumbnailChildView thumbnailView, final int position) {
        ...
        if (canvas != null) {
            ...
            if (thumbnailView.getThumbnailRect() != null) {
                int thumbnailOffsetPageWidth = thumbnailView.getThumbnailRect().width() /2 + UIUtil.convertDPtoPX(activity, 2);
                int thumbnailOffsetPageHeight = (int) (thumbnailView.getThumbnailRect().height())+ UIUtil.convertDPtoPX(activity, 2);//FIXME 시안 나오면 거기에 맞춰서 작업하자.
...
//                조금 작게 설정해야 썸네일이 안 잘리고 잘 그려진다..특히 달력은 아래 shadow2가 덮고 있어서 더 작게 설정해야 한다.
                int offsetSize = UIUtil.convertDPtoPX(activity,3);
                    thumbnailOffsetPageHeight -= offsetSize;
                    thumbnailOffsetPageWidth -= (offsetSize * ratioThumbnailOffsetWH);
        ...
        if (getCurrentPageIndex() == position) {
            thumbnailView.getOutline().setBackgroundResource(R.drawable.shape_image_border_transparency_photo_card_select);
        } else {
            thumbnailView.getOutline().setBackgroundResource(R.drawable.shape_image_tranparency_photo_card_border);
        }
