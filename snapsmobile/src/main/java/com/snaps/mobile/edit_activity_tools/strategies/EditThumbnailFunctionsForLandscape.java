 ...
 */
public class EditThumbnailFunctionsForLandscape implements IEditThumbnailBehaviorByOrientation {
    ...
    public void scrollToIdx(EditActivityThumbnailRecyclerView recyclerView, int moveType, int itemTotalCnt, int childIdx) {
        ...
        if (isSmoothScroll && moveType == EditActivityThumbnailUtils.PAGE_MOVE_TYPE_NEXT) {
            ...
            int offset = 0;
            if (item != null)
                offset = (recyclerView.getMeasuredHeight() / 2) - (recyclerView.getChildAt(0).getMeasuredHeight() / 2);
            else
                offset = (recyclerView.getMeasuredHeight() / 3);
