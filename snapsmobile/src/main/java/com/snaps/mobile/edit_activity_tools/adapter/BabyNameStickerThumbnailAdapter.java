 ...
 */
public class BabyNameStickerThumbnailAdapter extends BaseEditActivityThumbnailAdapter {
    private static final String TAG = BabyNameStickerThumbnailAdapter.class.getSimpleName();

    private Map<Integer, BabyNameStickerThumbnailHolder> m_mapThumbnailHolders = null;
        ...
        try {
            if (m_mapThumbnailHolders != null) {
                ...
                for (int key : m_mapThumbnailHolders.keySet()) {
                    BabyNameStickerThumbnailHolder holder = m_mapThumbnailHolders.get(key);
                    if (holder == null || holder.canvas == null) {
                        continue;
                    }
    ...
    public Map<Integer, BabyNameStickerThumbnailHolder> getThumbnailHolders() {
        return m_mapThumbnailHolders;
    }

    public BabyNameStickerThumbnailAdapter(Activity activity, SnapsProductEditInfo editInfo) {
    ...
    @Override
    public BabyNameStickerThumbnailHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        int layoutId = R.layout.bottomview_item_baby_name_card_dragable;

        View v = LayoutInflater.from(parent.getContext()).inflate(layoutId, parent, false);

        return new BabyNameStickerThumbnailHolder(v);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
        if (m_arrPageList == null || m_arrPageList.size() <= position) {
            return;
        }

        final BabyNameStickerThumbnailHolder holder = (BabyNameStickerThumbnailHolder) viewHolder;
        ...
        ViewGroup.LayoutParams imgParams2 = holder.canvasParentLy.getLayoutParams();
        ...
        if (imgParams.width != thumbnailRect.width() || imgParams.height != thumbnailRect.height()) {
            ...
            imgParams2.width = thumbnailRect.width();
            imgParams2.height = thumbnailRect.height();
            holder.imgLayout.setLayoutParams(imgParams);
            holder.canvasParentLy.setLayoutParams(imgParams2);
        }
    ...
    private void setTextSizeWithViewHolder(BabyNameStickerThumbnailHolder holder) {
        if (holder == null) {
            return;
        }
    ...
    public boolean isDiffThumbnailSize() {
        if (m_arrPageList == null || m_arrPageList.size() < 2) {
            return false;
        }
    ...
    private BabyNameStickerThumbnailHolder getThumbnailHolder(int position) {
    ...
    private void bottomViewItemClick(BabyNameStickerThumbnailHolder holder, boolean isClick) {
    ...
    public void onViewRecycled(RecyclerView.ViewHolder viewHolder) {
        BabyNameStickerThumbnailHolder holder = (BabyNameStickerThumbnailHolder) viewHolder;
    ...
    public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
        if (actionState == ItemTouchHelper.ACTION_STATE_DRAG) {
            ...
            if (snapsProductEditorAPI != null) {
                BabyNameStickerThumbnailHolder holder = getThumbnailHolder(m_iMovedPosition);
                if (holder != null) {
