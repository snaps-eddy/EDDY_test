...
public class SnapsPhotoPrintView extends ImageView {
	private static final String TAG = SnapsPhotoPrintView.class.getSimpleName();
	// 인화영역 크기...
	...
	public SnapsPhotoPrintView(Context context) {
		super(context);
		init();
	}

	public SnapsPhotoPrintView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public SnapsPhotoPrintView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void calculatorClipRect() {
		...
		if (!mIsViewRotate && mImgData != null && ((mImgData.ROTATE_ANGLE == 90) || (mImgData.ROTATE_ANGLE == 270))) {
	...
	void adjustCropRect(float cWidth, float cHeight) {
		...
		if (cInfo != null && CropInfo.CORP_ORIENT.NONE != cInfo.cropOrient) {
			...
			if (tempOrient == CORP_ORIENT.WIDTH) {
				Dlog.d("adjustCropRect() width angle:" + angle);
				if (angle == 0) {
					;
				} else if (angle == 1) {
					if (!mIsViewRotate)
				...
				Dlog.d("adjustCropRect() height angle:" + angle);
				if (angle == 0) {
					;
				} else if (angle == 1) {
					;
		...
		if (mIsTouch) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (orientValue == CORP_ORIENT.WIDTH) {
					moveX = mPrePos.x - event.getX();
					mPrePos.x = event.getX();
				} else {
					moveY = mPrePos.y - event.getY();
					mPrePos.y = event.getY();
				}
