 ...
 */
public class ImageSelectTrayEmptyShapeAdapter extends ImageSelectTrayBaseAdapter implements ISnapsDiaryRecyclerCustomAdapter {
    private static final String TAG = ImageSelectTrayEmptyShapeAdapter.class.getSimpleName();
    private ImageSelectTrayCellItem photoPrintDummyView = null;

    public ImageSelectTrayEmptyShapeAdapter(ImageSelectActivityV2 imageSelectActivityV2) {
        this(imageSelectActivityV2, imageSelectActivityV2);
    }

    protected ImageSelectTrayEmptyShapeAdapter(Context context, IImageSelectPublicMethods imageSelectPublicMethods) {
    ...
    private void setDummyHolder(ImageSelectAdapterHolders.TrayThumbnailItemHolder holder) {
        ...
        if (Config.isSnapsPhotoPrint() || Const_PRODUCT.isTransparencyPhotoCardProduct()) {
    ...
    public boolean checkExcessMaxPhoto() {
        ...
        if (holder != null) {
            ...
            if (maxSelectImgs > 0 && holder.getMapSize() > maxCount) { // 데이터// 추가
                if (Config.isSnapsSticker()) {
                    String homeSelectKind = "";
                    if (intentData != null) {
                        homeSelectKind = intentData.getHomeSelectKind();
                    }

                    if (homeSelectKind.equals("6")) {
                        MessageUtil.alert(context, R.string.maxcount_stick6);
                        return true;
                    } else if (homeSelectKind.equals("2")) {
                        MessageUtil.alert(context, R.string.maxcount_stick2);
                        return true;

                    } else if (homeSelectKind.equals("1")) {
                        MessageUtil.alert(context, R.string.maxcount_stick1);
                        return true;
                    }
                }
            }

            // 사진인화인경우 최대선택 갯수를 넘었을때 알림 메세지를 띄운다...
            if (Config.isSnapsPhotoPrint_kr() && holder.getMapSize() >= PHOTO_PRINT_MAX) {
                if (!isShowing) {
                    isShowing = true;
                    MessageUtil.alert(context, R.string.select_photo_maxcount, false, new ICustomDialogListener() {
                        @Override
