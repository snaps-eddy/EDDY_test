 ...
 */
public class ImageSelectTrayIdentifyPhotoAdapter extends ImageSelectTrayBaseAdapter implements ISnapsDiaryRecyclerCustomAdapter {
    private static final String TAG = ImageSelectTrayIdentifyPhotoAdapter.class.getSimpleName();
    private ImageSelectTrayCellItem photoPrintDummyView = null;

    public ImageSelectTrayIdentifyPhotoAdapter(ImageSelectActivityV2 imageSelectActivityV2) {
        this(imageSelectActivityV2, imageSelectActivityV2);
    }

    protected ImageSelectTrayIdentifyPhotoAdapter(Context context, IImageSelectPublicMethods imageSelectPublicMethods) {
    ...
    public int getEmptyCellCount() {
        ...
        return holder != null ? (1 - holder.getMapSize()) : 0;
    }
    ...
    public boolean insertPhotoThumbnailOnTrayItem(TrayAdapterInsertParam trayAdapterInsertParam) {
        if (isPageSyncLock() || trayAdapterInsertParam == null) return false;
        ...
        if (cellItemId >= 0) {
            ...
            if (cellItem != null) {
                cellItem.setImageKey(trayAdapterInsertParam.getImageMapKey());
                cellItem.setCellState(PHOTO_THUMBNAIL);
                cellItem.setNoPrint(trayAdapterInsertParam.getImageData().isNoPrint);
                notifyItemChanged(cellItem.getCellId());

                scrollToCenterTrayView(cellItem.getCellId());
            }

            setSelectedImageInfo(trayAdapterInsertParam.getImageMapKey());
