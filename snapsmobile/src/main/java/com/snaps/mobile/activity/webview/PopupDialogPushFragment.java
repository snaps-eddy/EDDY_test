...
public class PopupDialogPushFragment extends DialogFragment {
	private static final String TAG = PopupDialogPushFragment.class.getSimpleName();
	// 요구사항..
	// 받은 url을 호출한다.
	// 동영상을 플레이 할수 있도록 한다.
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View v = inflater.inflate(R.layout.dialog_popup_push_fragment_, container, false);
		...
		ImageView Pushclose = (ImageView) v.findViewById(R.id.push_bottom_close);
		...
		Pushcheck = (ImageView) v.findViewById(R.id.push_bottom_check);
	...
	public void saveDate() {
		...
		Setting.set(getActivity(), Const_VALUE.PUSH_FULL_CHECK_KEY, dateToday);
	}
	...
	class WebClient extends WebViewClient {
		...
		public boolean shouldOverrideUrlLoading(WebView view, String url) {
			...
			// snapsapp://cmd=gotoPage&pageNum=P0001
			if (url.startsWith("snapsapp://")) {
				...
				if (schemeHost != null && schemeHost.length() > 0) {
					if (schemeHost.equals("gotoPage")) {
						requestPushLog(url, hashmap.get("pageNum"));
	...
	void requestPushLog(final String fullUrl, final String pageNum) {
		...
		if (Config.getCHANNEL_CODE().equals(Config.CHANNEL_SNAPS_KOR)) {
			...
			ATask.executeVoidWithThreadPool(new OnTask() {
				...
				public void onPost() {
					...
					goHomeIntent.putExtra("gototarget", pageNum);

					goHomeIntent.putExtra("fullurl", fullUrl);
