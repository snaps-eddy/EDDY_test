...
import com.snaps.mobile.edit_activity_tools.adapter.PhotoCardActivityThumbnailAdapter;
...
public class PhotoCardEditor extends SnapsCardShapeProductEditor {
    private static final String TAG = PhotoCardEditor.class.getSimpleName();

    public PhotoCardEditor(FragmentActivity fragmentActivity) {
    ...
    public int getAutoSaveProductCode() {
        return IAutoSaveConstants.PRODUCT_TYPE_PHOTO_CARD;
    }
    ...
    public BaseEditActivityThumbnailAdapter createThumbnailAdapter() {
        return new PhotoCardActivityThumbnailAdapter(getActivity(), getEditInfo());
    }
    ...
    public void appendAddPageOnLoadedTemplate(SnapsTemplate template) {
        ...
        try {
            ...
            int maxImageCount = (template.getPages().get(2).getLayerLayouts().size());
            int addCount = (int) Math.ceil((double)count / maxImageCount / 2);
