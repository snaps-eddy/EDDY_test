...
public class SmartRecommendBookEditorBottomBGResListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	private static final String TAG = SmartRecommendBookEditorBottomBGResListAdapter.class.getSimpleName();
	...
	public SmartRecommendBookEditorBottomBGResListAdapter(Activity activity, boolean isLandscapeMode) {
	...
	private RecyclerView.ViewHolder getItemViewHolder(ViewGroup parent) {
		View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.smart_snaps_analysis_edit_bottom_bg_list_fragment_item, parent, false);
//		int columnCount = (isLandscapeMode ? Const_VALUE.IMAGE_GRID_COLS_LANDSCAPE : Const_VALUE.IMAGE_GRID_COLS);

//		int holderDimens = isLandscapeMode ? UIUtil.getScreenHeight(activity) : UIUtil.getScreenWidth(activity);
//		holderDimens /= columnCount;
		...
		lp.width = UIUtil.convertDPtoPX(activity, 80);
		lp.height = lp.width;
